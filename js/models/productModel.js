//Impoterer en hjælpefunktion til at lave fetch-kald (HTTP requests)
...
export const getDetails = async (product) => {
    //Bygger URL'en til API'et for at hente ét produkt vai dets ID
    const url = `http://localhost:4000/api/products/byId/${product}`

    //Sender GET-request til serveren og venter på data
    const data = await request(url)

    //Retunerer produktdata (fx navn, beskrivelse, pris osv.)
    return data
}